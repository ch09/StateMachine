...
public:
	...
	virtual void InvokeStateAction(StateMachine* sm, const EventData* data) const 
	{
		...
		// static_cast operator can be used for operations such as converting: 
		// Pointer to a base classe --> to a pointer to derived class 
		// Static_cast does not do a run time check 
		//So use static_cast only if we have a safe conversion 
...
/// Function-like macro 
/// "/" back to line! compiler understand that we still have some code
